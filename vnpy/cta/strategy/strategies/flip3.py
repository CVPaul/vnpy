...
class Flip3Strategy(CtaTemplate):
    """"""
    ...
    def on_bar(self, bar: BarData):
        ...
        cond_l = next_open - self.dnn > self.atr * self.k1
        cond_s = self.upp - next_open > self.atr * self.k1
        if self.mp == 0:
            if cond_l:
                ...
                self.write_log(f"buy:({self.mp},{self.enpp=},{self.long_loss_price=},{self.long_profit_price=},{self.volume=},{self.atr=})")
            elif cond_s:
                ...
                self.order_pair[loss_id] = prof_id
                self.order_pair[prof_id] = loss_id
                self.write_log(f"sell:({self.mp},{self.enpp=},{self.short_loss_price=},{self.short_profit_price=},{self.volume=},{self.atr=})")
        self.put_event()
